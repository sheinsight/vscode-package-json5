  ...
  public async resolveTask(_task: Task): Promise<Task | undefined> {
    ...
    if (npmTask) {
      ...
      if (kind.path) {
        ...
        packageJsonUri = _task.scope.uri.with({
          path: _task.scope.uri.path + "/package.json5",
        });
      }
  ...
  try {
    for (const folder of folders) {
      if (
        ...
        const relativePattern = new RelativePattern(folder, "**/package.json5");
...
export async function createTask(
  ...
  function getRelativePath(packageJsonUri: Uri): string {
    ...
    const absolutePath = packageJsonUri.path.substring(
      0,
      packageJsonUri.path.length - "package.json5".length
    );
...
export function getPackageJsonUriFromTask(task: Task): Uri | null {
  if (isWorkspaceFolder(task.scope)) {
    if (task.definition.path) {
      return Uri.file(
        path.join(task.scope.uri.fsPath, task.definition.path, "package.json5")
      );
    } else {
      return Uri.file(path.join(task.scope.uri.fsPath, "package.json5"));
    }
  }
  return null;
}
...
async function hasRootPackageJson(): Promise<boolean> {
  ...
  for (const folder of folders) {
    if (folder.uri.scheme === "file") {
      const packageJson = path.join(folder.uri.fsPath, "package.json5");
