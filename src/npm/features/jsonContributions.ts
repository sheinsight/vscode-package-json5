...
import { SyntaxKind, ScanError } from "vs-json5/@shared/constant";
...
export class JSONCompletionItemProvider implements CompletionItemProvider {
  ...
  private isLast(scanner: JSON5Scanner, offset: number): boolean {
  ...
  private hasColonAfter(scanner: JSON5Scanner, offset: number): boolean {
    scanner.setPosition(offset);
    // @ts-ignore
